...
class Level {
  constructor(grid = [], actors = []) {
    this.grid = grid;
    this.actors = actors;
    this.player = this.actors.find(function (el, i) {
    ...
    // this.theme = new Audio('fantasytheme.mp3');
    // this.theme.play();
  }
  ...
  obstacleAt(vectorPos, vectorSize) {
    ...
    const right = Math.ceil(vectorPos.x + vectorSize.x);
    const bottom = Math.ceil(vectorPos.y + vectorSize.y);
  ...
  noMoreActors(type) {
    ...
    for (let item of this.actors) {
      // console.log(111111, item.type, type)
      if (item.type === type) {
        return false
      }
    }
  ...
  playerTouched(type, actor) {
    ...
    switch (type) {
      ...
      case 'fireball':
        ...
        this.player.theme.pause();
        this.player.theme = new Audio('YOUDIED.mp3');
        this.player.theme.play();
        document.getElementsByClassName('player')[0].style.backgroundImage = "url('https://pp.userapi.com/c844721/v844721900/18e305/qqJDhYlVyoU.jpg')";
        document.getElementsByClassName('player')[0].style.backgroundSize = 'cover';
        this.player.size = new Vector(2, 3);
        timeStep = 0.008;
        this.status = 'lost';
        setTimeout(function () {
          timeStep = 0.02;
      ...
      case 'coin':
        ...
        if (this.player) this.player.size = new Vector(this.player.size.x + 0.1, this.player.size.y + 0.1), this.player.pos.y -= 0.1;
        if (actor) {
          ...
          if (this.noMoreActors('coin')) {
            ...
            if (this.player) {
              this.player.theme.pause();
              this.player.theme = new Audio('yee.mp3');
              this.player.theme.play();
            }
          }
        }
...
const level = new Level(grid, [goldCoin, bronzeCoin, player1, fireball]);
...
const parser1 = new LevelParser(actorDict1);
runGame(schemas, parser1, DOMDisplay)
  .then(() => {
    let player = new Audio('hb - 04.03.2019, 20.30.mp3');
    player.play();
    let game = document.querySelector('.secret');
    game.style.width = '1200px';
    game.style.height = '800px';
    game.style.backgroundImage = "url('100_1046.JPG')";
    game.style.backgroundSize = "cover";

    setTimeout(function () {
      alert('Бля Саня, ты могучий, ты мощный, ты неповторимый, ты собрал все золото, всю метеоритную руду, ты достоин быть паладином и месить драконов, ну и конечно ..... бугаи то ходят? а что говорят нигеры? С днюхой :)')
    }, 2000)
  })
...
function onKey(event) {
  let code = ["KeyK", "KeyJ", "BracketLeft", "KeyK", "KeyJ", "BracketLeft", "KeyK", "KeyJ", "BracketLeft"];
  event.code === code[counter] ? counter++ : counter = 0;
  if (counter === code.length) alert('Ты пёс');
}
